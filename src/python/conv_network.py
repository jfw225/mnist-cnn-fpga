...
def forward(x):
    ...
    x, w1, b1, w3, b3, w5, b5 = [
        i.astype(np.float16) for i in (x, w1, b1, w3, b3, w5, b5)]
    # (1, 27, 27, 8) = (1, 28, 28) , (2, 2, 8)
    Z1 = convolve(x, w1).astype(np.float16)
    ...
    Z3 = arr[0]
    for i in range(1, 8):
        Z3 += arr[i].astype(np.float16)
    A3 = (Z3 + b3).astype(np.float16)  # (1, 12, 12, 8) = (1, 12, 12, 8)
    # print('Layer 3: ', np.max(Z3), np.min(Z3),
    #   np.max(Z3 + b3), np.min(Z3 + b3))
    Z4 = maxpool(A3).astype(np.float16)  # (1, 6, 6, 8) = (1, 12, 12, 8)
    # print('Layer 4: ', np.max(Z4), np.min(Z4))
    # (1, 10) = (1, 288) (288, 10)
    Z5 = Z4.reshape(Z4.shape[0], -1).astype(np.float16) @ w5.astype(np.float16)
