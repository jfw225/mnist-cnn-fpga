...
class Maxpool_FC(Layer):
    ...
    def __init__(
        ...
        # create a register to keep the output of the dps
        self.dp_arr = self.var(dtype=V_RegArray,
                               width=self.width,
    ...
    def generate(self) -> V_Block:
        ...
        vsm = V_StateMachine(_StReset, _StWaitValid, _StResetWindow,
                             _StSetInpAddr, _StWindowBuffer, _StSetWindow, _StMaxReset,
                             _StGetMaxOverWindow, _StMaxBuffer, _StDpBuffer,
                             _StFindMax, _StIncMaxpool, _StWriteData, _StWriteBuffer)

        sm = self.signed_max

        wlen = self.output_mem.size
        return super().generate(vsm, V_Block(
            ...
            *mult.instantiate(
                ...
                (V_Empty(), mult.done),
                (V_Empty(), mult.ready),
                (self.max_out, mult.input_ports[0]),
                (self.weights[
                    (self.out_y * (self.x // 2) * self.n * wlen) +
                    (self.out_x * self.n * wlen) +
                    (self.wc * wlen) +
                    self.dpi], mult.input_ports[1]),
...
class _StReset(V_State):
    ...
    def generate(self, m: Maxpool_FC) -> V_Block:
        ...
        return V_Block(
            ...
            *[m.dp_arr[i].set(V_Low) for i in range(m.dp_arr.size)],
            m.dp.set(V_Low),
...
class _StFindMax(V_State):
    ...
    def generate(self, m: Maxpool_FC) -> V_Block:
        max_dpi = m.output_mem.size - 1

        return V_Block(
            m.dp_arr[m.dpi].set(m.dp_arr[m.dpi] + m.prod),
...
class _StWriteData(V_State):
    ...
    def generate(self, m: Maxpool_FC) -> V_Block:
        ...
        return V_Block(
            m.out_we.set(V_Low),
            m.out_data.set(m.dp_arr[m.out_addr] + m.biases[m.out_addr]),
